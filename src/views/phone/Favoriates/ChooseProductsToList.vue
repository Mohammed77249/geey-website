<template>
  <div class="bg-gray-50 w-full min-h-screen h-full">
    ...
    >
      <div v-if="ListName != ''" class="flex items-center justify-between">
        <!-- back button -->
      ...
      <div v-else class="grid grid-cols-12 items-center justify-between">
        <!-- back button -->
        ...
        >
          ...
          <div class="flex items-center justify-between mt-2">
            ...
            <div
              ...
              class="cursor-pointer w-8 flex items-center justify-center rounded-full"
            >
              <svg
                width="20"
                height="20"
          ...
          <div class="flex items-center justify-between">
            <div class="flex gap-2 items-center">
              <p class="font-sembold text-primary-900 text-xs">
                {{ product.base_price }}
              </p>
              <div class="">
        ...
        >
          ...
          <div class="flex items-center gap-5">
            <span
              ...
              class="text-md font-semibold text-primary-900"
            >
              حذف</span
            >
            ...
            >
              {{ ListName != '' ? ' نسخ الى القائمة ' : ' أضف الى القائمة ' }}
            </button>
...
const LoaderDatacomp = defineAsyncComponent(
  () => import('@/components/LoaderDatacomp.vue'),
)
...
const deleteProducts = async () => {
  if (ListName.value != '') {
    // delete from list
    const selectedIds = ListProducts.value
      .filter(p => p.selected)
      .map(p => p.id)

    if (selectedIds.length > 0) {
      const result = await storeFav.deleteListProductFavorite(
        selectedIds,
        ListId.value,
      )
      ...
      if (result) {
        ...
        ListProducts.value = storeFav.getProductsInList
      } else {
...
onMounted(async () => {
  ListName.value = decodeURIComponent(route.query.from || '')
  ListId.value = decodeURIComponent(route.query.id || null)

  if (ListName.value != '') {
    await storeFav.fetchProductInList(ListId.value)
    ListProducts.value = storeFav.getProductsInList
    CopyOrNot.value = true
  } else {
    await storeFav.fetchProductInfavorite()
    ListProducts.value = storeFav.getProductsInFavorite
    CopyOrNot.value = false
  }
});
</script>
